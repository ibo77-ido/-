...
Current Workflow: Phase3 烧制 UI 接入 (Project/烧制UI/)

Current Task: 烧制UI Task06 Phase3SceneBuilder 检测分支 — PASS

Current Status: Task06 PASS（ArtRoot_Firing 检测分支已添加，美术模式下跳过旧占位控件创建）
...
Phase8: COMPLETE（P8-00~P8-19 ALL PASS）
Phase3 烧制UI: COMPLETE（Task01~06 ALL PASS）
...
| 烧制UI-T01 | FiringSystem 扩展（低风门降温 + 接口） | PASS |
| 烧制UI-T02 | 场景层级搭建（ArtRoot_Firing + 视频 + 风门 + 按钮） | PASS |
| 烧制UI-T03 | FiringPanelController 状态机（3态 + 视频两段式 + Refresh分离） | PASS |
| 烧制UI-T04 | 风门滚轮交互（9档切换 + 序列帧缓存 + Overlay检测） | PASS |
| 烧制UI-T05 | 温度条视频驱动（progress独立 + 倒放 + 强制开窑次品） | PASS |
| 烧制UI-T06 | Phase3SceneBuilder 检测分支（ArtRoot_Firing 检测） | PASS |
...
2026-06-19（Phase3 烧制UI Task06 PASS — 烧制 UI 接入工作流 COMPLETE，6 个 Task 全部 PASS）
...
### Phase9 NavMesh + role-mobile Movement Alignment - In Progress 2026-06-18

**Scene**: `Assets/Phase9/Scenes/SampleScene.unity`

**Current Status**: Phase9 NavMesh bake PASS; role-mobile Step 1 and Step 2 completed. Step 3 pending.

| Area | Result |
|---|---|
| XZ alignment | `静态层` and `NavMesh-walkable` share position/rotation/scale |
| Bake source | `NavMeshWalkableBakeMesh_FromAlpha` generated from `NavMesh-walkable.png` alpha mask |
| Legacy bake mesh | `NavMeshWalkableBakeMesh_FromSprite` removed from Road layer and MeshCollider disabled |
| Agent type | `Phase9SmallAgent` created and used by `Phase9_NavMeshSurface` and `女主` |
| Agent radius / height | radius `0.06`, height `0.5` |
| NavMesh report | PASS, `2467` vertices, `1163` triangles |
| NavMesh Y | `3.261 .. 3.261` |
| Reachability smoke | 4 validation entry points reachable in normalizer report |

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase9/Scripts/Editor/Phase9XZSceneNormalizer.cs` | Replaced Sprite Tight Mesh bake with alpha-mask rectangle mesh; created Phase9SmallAgent; disabled legacy FromSprite bake object |
| `Assets/Generated/NavMesh/NavMeshWalkableBakeMesh_FromAlpha.asset` | Created generated Road-layer bake mesh asset |
| `ProjectSettings/NavMeshAreas.asset` | Added `Phase9SmallAgent` |
| `Assets/地图素材/地图分层/NavMesh-walkable.png.meta` | `isReadable` enabled so alpha pixels can be read |
| `Assets/Phase9/Scenes/SampleScene.unity` | Saved Phase9 bake objects, surface agent, and bridge serialized reference updates |
| `Assets/Phase9/role-mobile.md` | Created role-mobile execution plan |

**role-mobile Step 1 - Completed**:

| Check | Result |
|---|---|
| Movement chain identified | `Phase9InteractionBridge -> PlayerCharacter -> MovementController` |
| Input handler | `Phase9InteractionBridge.HandleWorldClickMove()` |
| Target resolver | `TryResolveMoveTarget()` |
| Runtime movement | `MovementController` manual NavMesh path corner movement |
| Player agent | `Phase9SmallAgent`, radius `0.06`, height `0.5`, `updatePosition=false`, `updateRotation=false` |
| Four core interact objects | `Order-interact`, `Shape-interact`, `Glaze-interact`, `Kiln-interact` exist |

**role-mobile Step 2 - Completed**:

| Fix | Result |
|---|---|
| `walkableAreaName` default | changed from `NavMesh-walkable (1)` to `NavMesh-walkable` |
| `_BridgeRoot` scene instance | serialized `walkableAreaName` updated to `NavMesh-walkable` |
| Old click coverage check | removed `sprite.vertices / sprite.triangles` Tight Mesh dependency |
| Current click gate | renderer bounds coarse check + `NavMesh.SamplePosition` + `NavMesh.CalculatePath(PathComplete)` |
| Verification | `walkableAreaName=NavMesh-walkable`, resolves correctly, NavMesh `2467/1163` |

**Known issue**:

- Unity console still logs old `PPtr cast failed when dereferencing! Casting from Sprite to Texture!`; not a compile error and Phase9 bake still PASS.

**Next Recommended Task**: role-mobile Step 4 - 验证 Phase3 四个核心交互运行时流程 + Step 5 匹配运行时移动逻辑 + Step 6 处理移动遮挡关系。

### Phase9 role-mobile Step 3 - Completed 2026-06-18

**Scene**: `Assets/Phase9/Scenes/SampleScene.unity`

**Current Status**: Step 3 NavMesh 蓝色连通区域覆盖验证 PASS。

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase9/Scripts/Editor/Phase9NavMeshCoverageValidator.cs` | Created Editor-only 验证工具，菜单 `Phase9/Verify/NavMesh Coverage Report` |
| `Assets/Screenshots/phase9-navmesh-coverage-report.txt` | Generated coverage report |

**role-mobile Step 3 - Completed**:

| Check | Result |
|---|---|
| NavMesh triangulation | vertices=2467, triangles=1163, navY=3.2609 |
| 女主 origin | (-0.9500, 3.2609, -2.3600), SamplePosition PASS |
| 采样点总数 | 300 (三角形质心采样) |
| Coverage reachable | 300 |
| Coverage unreachable | 0 |
| Order-interact | PASS, distance=2.345, pos=(-3.1365, 3.2609, -1.5118) |
| Shape-interact | PASS, distance=3.406, pos=(-1.8665, 3.2609, 0.9205) |
| Glaze-interact | PASS, distance=4.230, pos=(-0.6600, 3.2609, 1.8600) |
| Kiln-interact | PASS, distance=3.807, pos=(1.1660, 3.2609, 0.8044) |
| Verdict | Coverage=PASS Interaction=PASS Overall=PASS |

**Serialized References Changed**: NONE

**Scene Mutation**: NONE

**Known issue** (承袭): Unity console 仍可能日志 `PPtr cast failed when dereferencing! Casting from Sprite to Texture!`;非编译错误，Phase9 bake / coverage 验证均 PASS。

**Next Recommended Task**: role-mobile Step 4 - 验证 Phase3 四个核心交互运行时流程（女主到达后按 E 进入各模块，完成一轮 Order→Shape→Glaze→Kiln 不需手动拖物体）。


### mobile-转向 Step 1 - Completed 2026-06-18

**Task**: Animator 数据结构补全 + 为 Step 4 打通 Move 驱动接口

**Scene**: `Assets/Phase9/Scenes/SampleScene.unity` (场景参数未修改)

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Animation/Female Lead Animator Controller.controller` | 新增 3 参数 (MoveX/MoveY/Speed) + Blend Tree "Move" |
| `Assets/Phase9/Scripts/HeroPathMover.cs` | Created Runtime 脚本骨架 |

### mobile-转向 Step 4 - Completed 2026-06-18

**Task**: HeroPathMover 移动逻辑实现 + 场景绑定 + 运行时验证

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase9/Scripts/HeroPathMover.cs` | 实现 StartPath/Stop/Update 完整逻辑；NavMeshAgent runtime 冻结；rotation 单点控制 |

**Scene Mutation**:
- HeroPathMover 组件挂载到 HeroineRoot (4 引用绑定)
- VisualRoot.Animator.enabled: false → true

**WF_03_VERIFICATION: PASS** — 15 项验收通过（路径预采样、位置推进、Animator 参数写入、Stop 衰减、rotation 单点控制、状态恢复）


### Phase3 订单 UI 第一轮接入 - Completed 2026-06-19

**Task**: 将 Panel_Order 接入美术素材（订单面板 文字.png + 接单按钮.png），改造 OrderPanelController.ShowOrder 移除标签拼接，保存 OrderPanelArt.prefab

**方案文件**: `Project/Phase3订单UI接入方案.md`

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase3/Prefabs/UI/OrderPanelArt.prefab` | Created Prefab from Panel_Order 根对象（保留根保引用） |
| `Assets/Phase3/Scripts/UI/OrderPanelController.cs` | ShowOrder 移除"订单："等标签拼接，只显示值 |
| `Assets/Phase3/Scenes/Phase3_Prototype.unity` | Panel_Order 改造：Image→美术图, 删除冗余节点+Btn_Accept/Text_Label, sizeDelta→(900,720) |

**核心决策（用户修正版 APPROVED）**:
- 保留旧 Panel_Order 根对象，只替换子层级和组件，再保存为 Prefab（最大程度保住 GameManager.panelOrder 引用）
- Btn_Accept 下 Text_Label 必须删除（避免与接单按钮.png 自带"接单"重字）
- 子节点数量不卡死（验收关键：背景+4 值文本+Btn_Accept 直接子节点+引用正确）

**Serialized References Changed**:
- [NEW] HeroPathMover on HeroineRoot (Step 4)
- [NEW] OrderPanelArt.prefab at Assets/Phase3/Prefabs/UI/
- [MODIFIED] GameManager.orderPanelController → 重新绑定（Prefab 连接导致丢失，已修复）
- [PARAM MODIFY] Panel_Order Image.sprite=订单面板文字, color=(1,1,1), raycastTarget=false
- [PARAM MODIFY] Btn_Accept Image.sprite=接单按钮, color=(1,1,1)
- [PARAM MODIFY] Panel_Order sizeDelta=(900,720)
- [PARAM MODIFY] VisualRoot.Animator.enabled=true (Step 4)
- [DELETED] Text_OrderPanel, Text_OrderLabel, Btn_Accept/Text_Label

**Scene Mutation**:
- Modified Panel_Order 子层级（7→5 子节点）
- Modified Panel_Order / Btn_Accept Image 组件
- Modified Panel_Order RectTransform sizeDelta
- Saved as OrderPanelArt.prefab

**Acceptance Check (WF_03_VERIFICATION PASS)**:

| # | Check | Result |
|---|-------|--------|
| 1 | 编译 0 Error | ✓ |
| 2 | OrderPanelArt.prefab 创建 | ✓ |
| 3 | Panel_Order Image.sprite=订单面板 文字 | ✓ |
| 4 | Btn_Accept Image.sprite=接单按钮 | ✓ |
| 5 | Btn_Accept childCount=0 (Text_Label 已删) | ✓ |
| 6 | ShowOrder 只显示值（无标签前缀） | ✓ ("青釉碗") |
| 7 | GM.panelOrder 引用完整 | ✓ (保留根对象策略成功) |
| 8 | GM.orderPanelController 重新绑定 | ✓ (Prefab 连接后修复) |
| 9 | Phase3 Play Mode 接单流程 | ✓ (点击→GameState.Shape, 面板切换) |
| 10 | Phase9 桥接加载 Phase3 后面板 | ✓ (Additive 加载, panelOrder 正确) |
| 11 | Phase9 桥接美术生效 | ✓ (Image.sprite=订单面板文字) |
| 12 | Console 无 Error | ✓ (仅 Sprite→Texture Warning) |

**Risks**:
1. Text 位置基于估算（50,180)/(50,80)/(50,-20)/(50,-120)，未对照背景图横线精确校准，可能需微调
2. Panel_Order sizeDelta=(900,720) 可能对 Phase9 CenterGameplayPanels 居中后过大（CenterGameplayPanels 不设 sizeDelta）
3. preserveAspect=true 在非正方形容器(900x720)中可能导致图片缩放不均匀
...
或继续 Phase3 订单 UI 第二轮优化（文本位置微调 + 分辨率验证 + Phase9 居中超屏修复）

---

### Phase3 烧制 UI Task01 FiringSystem 扩展 — Completed 2026-06-19

**Task**: 扩展 FiringSystem 温度计算，新增低风门降温分支 + 3 个接口，为温度条视频驱动和次品判定提供数值支持。

**方案文件**: `Project/烧制UI/Task01_FiringSystem扩展.md`
...
**代码变更**:
- 新增 3 个 SerializeField: `lowWindThreshold`(0.3) / `lowWindDurationThreshold`(3) / `temperatureDropPerSecond`(30)
- 新增 2 个私有字段: `lowWindTimer` / `isTemperatureDropping`
- 新增 2 个只读接口: `IsTemperatureDropping` / `WindValue`
- 新增 `ForceUnderfiredOpen()` 方法（温度压0 + StopFiring，供温度条倒放到0帧调用）
- `Update()` 扩展低风门降温分支: windValue<0.3 持续3s → 降温30/s
- `ResetFiring()` / `StartFiring()` 同步清零新增字段

**Serialized References Changed**:
- [NEW SerializeField] FiringSystem: lowWindThreshold / lowWindDurationThreshold / temperatureDropPerSecond (均有默认值，无需 Inspector 操作)
...
|---|-------|--------|
| 1 | 编译通过 | ✓ |
| 2 | windValue=1 温度上升 50/s | ✓ |
| 3 | windValue=0.2 持续<3s 仍上升 | ✓ |
| 4 | windValue=0.2 持续>=3s 降温 30/s, IsTemperatureDropping=true | ✓ |
| 5 | windValue 恢复>=0.3 立即 IsTemperatureDropping=false | ✓ |
| 6 | ForceUnderfiredOpen(): temp=0, isFiring=false, isTemperatureDropping=false | ✓ |
| 7 | ResetFiring()/StartFiring() 清零新增字段 | ✓ |
| 8 | 既有接口签名不变 (AddFuel/StopFiring/GetFireScore/CalculateScore 等) | ✓ |

**用户风险提示评估**:
- 风险 A (isTemperatureDropping 即时切换缺余热 decay): 评估为 Task05 表现层问题，Task01 不改，已记入 Task05 备忘
- 风险 B (lowWindTimer deltaTime 上限保护): 评估项目无 timeScale 操作，`Mathf.Max(0,timer)` 无实际作用，保持现状；未来引入全局暂停时再统一收口

**Risks**:
1. isTemperatureDropping 即时切换，未来若做"余热惯性"UI 表现需在 Task05 加 decay window
2. lowWindTimer 无 deltaTime 上限保护，若未来引入 timeScale=0 暂停机制需统一收口时间推进

**Next Recommended Task**: 烧制UI Task02 场景层级搭建

---

### Phase3 烧制 UI Task02 场景层级搭建 — Completed 2026-06-19

**Task**: 在 Panel_Firing 下搭建 ArtRoot_Firing 美术层级，配置 4 个 VideoPlayer + RenderTexture + 风门节点 + 按钮换皮。通过 UnityMCP 在 Editor 直接操作，避免手动改场景 YAML。

**方案文件**: `Project/烧制UI/Task02_场景层级搭建.md`

**执行方式**: UnityMCP（manage_gameobject / manage_components / manage_asset / execute_code）

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase3/UI/烧窑/RT_Firing.renderTexture` | CREATED (1920x1080) |
| `Assets/Phase3/UI/烧窑/RT_Temperature.renderTexture` | CREATED (640x80) |
| `Assets/Phase3/Scenes/Phase3_Prototype.unity` | MODIFIED (场景层级扩展) |

**Scene Mutation**:
```
Panel_Firing (RectTransform 全屏 stretch)
├─ ArtRoot_Firing (sibling index=0, 渲染最下层, 全屏 stretch)
│  ├─ StaticFallback_Image (Image, sprite=底图 阶段1.png, raycastTarget=false)
│  ├─ FiringVideo_RawImage (RawImage, texture=RT_Firing, raycastTarget=false)
│  │  ├─ FireStart (VideoPlayer: 火焰动画.mp4, isLooping=false, playOnAwake=false, audio=None, targetTexture=RT_Firing)
│  │  ├─ FiringLoop (VideoPlayer: 循环.mp4, isLooping=true, playOnAwake=false, audio=None, targetTexture=RT_Firing)
│  │  └─ ClosingKiln (VideoPlayer: 关窑动画.mp4, isLooping=false, playOnAwake=false, audio=None, targetTexture=RT_Firing)
│  └─ TemperatureVideo_RawImage (RawImage, texture=RT_Temperature, raycastTarget=false, 底部居中 640x80)
│     └─ Temperature (VideoPlayer: 温度条变化.mp4, isLooping=false, playOnAwake=false, audio=None, targetTexture=RT_Temperature)
├─ 旧控件 inactive (8个): Text_FiringPanel/Label/Temperature/Zone/FireScore/Status, Slider_Wind, Btn_Window
├─ Btn_Fuel (active, sprite=添燃料按钮.png, 左下 -780,-440)
│  └─ Text_Label (inactive)
├─ Btn_Stop (active, sprite=停火按钮.png, 右下 780,-440)
│  └─ Text_Label (inactive)
├─ Btn_OpenKiln (inactive, sprite=打开按钮.png, 底部居中 0,-440)
│  └─ Text_Label (inactive)
├─ WindDoor_Image (Image, sprite=风门/1.png, 右侧中下 780,-100, 200x300, raycastTarget=true)
└─ WindDoor_HoverArea (Image 透明 alpha=0, 与 WindDoor_Image 重合, raycastTarget=false)
```
...
| # | Check | Result |
|---|-------|--------|
| 1 | ArtRoot_Firing 存在且为 Panel_Firing 第一个子节点 | ✓ |
| 2 | 4 个 VideoPlayer 绑定对应 mp4 clip | ✓ |
| 3 | RT_Firing 被 3 个主画面 VideoPlayer 共享 | ✓ |
| 4 | RT_Temperature 被温度条 VideoPlayer 使用 | ✓ |
| 5 | WindDoor_Image 位于右侧中下留白区 | ✓ |
| 6 | WindDoor_HoverArea 与 WindDoor_Image 重合 | ✓ |
| 7 | 3 个按钮换美术 png | ✓ |
| 8 | 8 个旧控件 inactive (不删除) | ✓ |
| 9 | 按钮 Text 子节点 inactive, 本体保留 (Btn_OpenKiln 初始 inactive) | ✓ |
| 10 | 所有 VideoPlayer playOnAwake=false, audioOutputMode=None | ✓ |
| 11 | ArtRoot_Firing 在 sibling index=0 (视觉最下层) | ✓ |
| 12 | StaticFallback/FiringVideo/TemperatureVideo raycastTarget=false | ✓ |
| 13 | WindDoor_HoverArea raycastTarget=false (Task04 用主动检测) | ✓ |
| 14 | 场景已保存 | ✓ |

**Risks**:
1. 节点 RectTransform 位置为初始估算值，需 Editor 对照参考图精调
2. VideoPlayer 共享 RT_Firing，同时 Play 会冲突——Task03 确保同一时刻只 Play 一个
3. 旧控件 inactive 不删除，FiringPanelController 既有 SerializeField 引用仍指向它们——Task03 扩展时保留兼容
4. StaticFallback_Image 在视频上层会遮挡视频——层级顺序已确保 StaticFallback 在 FiringVideo 之前(渲染在下层)，正确

**Next Recommended Task**: 烧制UI Task03 FiringPanelController 状态机

---

### Phase3 烧制 UI Task03 FiringPanelController 状态机 — Completed 2026-06-19

**Task**: 扩展 FiringPanelController，实现 3 态状态机 + 主画面视频两段式播放 + Refresh 缓存优化。

**方案文件**: `Project/烧制UI/Task03_FiringPanelController状态机.md`
**实现报告**: `Project/烧制UI/Task03_实现报告.md`

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase3/Scripts/UI/FiringPanelController.cs` | MODIFIED (重写) |

**核心架构变更**:
- 加 `using UnityEngine.Video;` 解决 VideoPlayer 编译
- `EnterState(state, force=false)` 唯一状态入口，force 参数替代哨兵值
- Refresh 职责分离：只做连续刷新（文本/IsFiring检测），不做状态切换
- OnStopButtonClicked 只发起 EnterState(ClosingKiln)，按钮显隐交给 EnterState
- ResetPanel 用 force:true 强制重置
- 视频两段式：PlayFireStartSequence → loopPointReached → FiringLoop
- HideLegacyControls() 在 Start 时隐藏旧控件
- UnregisterVideoCallbacks() 在 OnDestroy 清理回调

**Serialized References Changed**:
- [NEW SerializeField] 13 个新字段全部通过 UnityMCP 反射设置并验证
  - 4 个 VideoPlayer: fireStartPlayer/firingLoopPlayer/closingKilnPlayer/temperatureVideoPlayer
  - 2 个 RawImage: firingVideoImage/temperatureVideoImage
  - 1 个 Image: staticFallbackImage
  - 3 个 GameObject: fuelButtonRoot/stopButtonRoot/openButtonRoot
  - 2 个 风门: windDoorImage/windDoorHoverArea
  - 1 个 Sprite[]: windDoorFrames (9帧)

**Scene Mutation**: NONE（仅改脚本 + Inspector 引用）
...
**Risks**:
1. VideoPlayer loopPointReached 回调运行时需验证无缝衔接
2. 3 个 VideoPlayer 共享 RT_Firing，Stop→Play 顺序已确保
3. 反射设置的字段已通过 SetDirty + SaveScene 序列化到场景文件

**Next Recommended Task**: 烧制UI Task04 风门滚轮交互 或 Task05 温度条视频驱动

---

### Phase3 烧制 UI Task04 风门滚轮交互 — Completed 2026-06-19

**Task**: 实现风门序列帧切换 + 鼠标中键滚轮交互。9 档步进，带缓存。

**方案文件**: `Project/烧制UI/Task04_风门滚轮交互.md`

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase3/Scripts/UI/FiringPanelController.cs` | MODIFIED (追加风门逻辑) |

**核心改动**:
- 新增 `lastWindFrame` 缓存字段（第 49 行）
- Update 追加 `UpdateWindDoorScroll()` + `RefreshWindDoorFrame()` 调用（第 92-93 行）
- `UpdateWindDoorScroll()`：滚轮检测 + 1/8 步进 + SetWindValue（第 147 行）
  - 只在 FiringActive 态可操作
  - RectTransformUtility.RectangleContainsScreenPoint（Overlay 模式 camera=null）
- `RefreshWindDoorFrame()`：windValue→frameIndex 映射 + 带缓存切 sprite（第 179 行）

**Serialized References Changed**: NONE（风门字段已在 Task03 绑定）
...
|---|-------|--------|
| 1 | 编译通过 | ✓ |
| 2 | lastWindFrame 缓存字段已添加 | ✓ (grep 行 49) |
| 3 | UpdateWindDoorScroll 方法已添加 | ✓ (grep 行 147) |
| 4 | RefreshWindDoorFrame 方法已添加 | ✓ (grep 行 179) |
| 5 | Update 中调用两个新方法 | ✓ (grep 行 92-93) |
| 6 | windDoorImage 引用已绑定 | ✓ (验证 true) |
| 7 | windDoorFrames 9 帧 Sprite 已绑定 | ✓ (验证 count=9) |
| 8 | windDoorHoverArea 引用已绑定 | ✓ (验证 true) |
| 9 | Canvas renderMode=ScreenSpaceOverlay, camera=null 正确 | ✓ |
| 10 | 运行时：滚轮 9 档切换 | ⏳ 待验证 |

**Risks**:
1. 低风险：Refresh() 的 IsFiring 变更触发 EnterState(FiringActive, true) 与 ResetPanel() 可能导致起势视频轻微重播——运行时若观察到回跳再收紧进入条件
2. 触屏设备无滚轮，第一版不处理

**Next Recommended Task**: 烧制UI Task05 温度条视频驱动

---

### Phase3 烧制 UI Task05 温度条视频驱动 — Completed 2026-06-19

**Task**: Controller 内部维护 temperatureBarProgress（0~1），按视频自身帧率线性推进；风门低进风持续 3s 时倒放；倒放到 0 帧触发强制开窑次品分支。

**方案文件**: `Project/烧制UI/Task05_温度条视频驱动.md`（含 Task01 风险 A 余热 decay 备忘）

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase3/Scripts/UI/FiringPanelController.cs` | MODIFIED (追加温度条视频驱动逻辑) |

**核心改动**:
- 新增 2 个 SerializeField: `temperatureBarForwardSpeed`(1) / `temperatureBarReverseSpeed`(1)
- 新增 4 个 private 字段: `temperatureBarProgress` / `isReversing` / `lastTemperatureBarFrame` / `hasTriggeredForceOpen`
- 新增 3 个方法:
  - `InitTemperatureVideo()` — 进入 FiringActive 时初始化温度条视频到 0 帧
  - `UpdateTemperatureVideo()` — 每帧驱动：IsTemperatureDropping 决定正向/倒放，progress 映射视频帧 seek（带缓存）
  - `TriggerForceUnderfiredOpen()` — 倒放到 0 触发：ForceUnderfiredOpen + 跳过 ClosingKiln + GoToResult
- Update 追加 `UpdateTemperatureVideo()` 调用
- EnterState FiringActive 分支追加 `InitTemperatureVideo()`
- ResetPanel 追加 `lastWindFrame` 重置

**关键逻辑**:
- `temperatureBarProgress` 完全独立于 `CurrentTemperature`，是 Controller 内部 0~1 进度
- 倒放由 `FiringSystem.IsTemperatureDropping` 驱动（windValue<0.3 持续3s）
- 倒放到 0 → `ForceUnderfiredOpen()` → 温度=0 → 分数=0 → 天然次品
- 第一版不做余热 decay window（Task01 风险 A 备忘），若视觉突变明显再加

**Serialized References Changed**:
- [NEW SerializeField] temperatureBarForwardSpeed (float, 默认 1)
- [NEW SerializeField] temperatureBarReverseSpeed (float, 默认 1)
- temperatureVideoPlayer/temperatureVideoImage 已在 Task03 绑定
...
| 1 | 编译通过 | ✓ |
| 2 | 9 个新成员已添加（grep 验证） | ✓ |
| 3 | Update 追加 UpdateTemperatureVideo() | ✓ (行 103) |
| 4 | EnterState FiringActive 追加 InitTemperatureVideo() | ✓ (行 120) |
| 5 | ResetPanel 追加 lastWindFrame 重置 | ✓ (行 467) |
| 6 | temperatureVideoPlayer 已绑定 | ✓ (Task03) |
| 7 | temperatureVideoImage 已绑定 | ✓ (Task03) |
| 8 | 运行时：正向推进 | ⏳ 待验证 |
| 9 | 运行时：倒放 + 强制开窑次品 | ⏳ 待验证 |

**Risks**:
1. VideoPlayer.frame 手动 seek 每帧调用可能有性能开销——带 lastTemperatureBarFrame 缓存
2. temperatureBarProgress 正向到 1 后停止（不循环）——需实测确认末帧表现
3. 倒放/正向速度默认 1（progress/秒），需实测调整
4. 余热 decay window 第一版不做（Task01 风险 A 备忘），若视觉突变明显再加

**Next Recommended Task**: 烧制UI Task06 Phase3SceneBuilder 检测分支

---

### Phase3 烧制 UI Task06 Phase3SceneBuilder 检测分支 — Completed 2026-06-19

**Task**: 修改 `EnsureFiringPanelControls()`，检测 `ArtRoot_Firing` 节点存在时跳过旧占位控件创建，避免 Builder 污染美术面板。

**方案文件**: `Project/烧制UI/Task06_Phase3SceneBuilder检测分支.md`

**Files / Assets changed**:

| Path | Change |
|---|---|
| `Assets/Phase3/Editor/Phase3SceneBuilder.cs` | MODIFIED (追加 ArtRoot_Firing 检测分支) |

**核心改动**:
- 在 `EnsureFiringPanelControls()` 方法开头（第 133-143 行）插入检测分支
- `panel.Find("ArtRoot_Firing") != null` → 仅确保 FiringPanelController 存在，return（跳过旧占位控件）
- 无 ArtRoot_Firing → 走原有逻辑（创建 Text_Zone/Text_FireScore/Text_Status/Slider_Wind/Btn_*）

**不改动部分**:
- Order/Shape/Glaze/Result 面板的 Builder 逻辑
- 原 Panel_Firing 无 ArtRoot_Firing 时的占位控件创建逻辑
- 其他所有 Editor 方法

**Serialized References Changed**: NONE

**Scene Mutation**: NONE（仅改 Editor 脚本逻辑）
...
|---|-------|--------|
| 1 | 编译通过 | ✓ |
| 2 | ArtRoot_Firing 检测分支已添加 | ✓ (行 135) |
| 3 | 美术模式下仅确保 FiringPanelController | ✓ (行 137-141) |
| 4 | 场景验证：artRootExists=true, hasController=true, 无重复控件 | ✓ |
| 5 | 旧占位控件创建逻辑保留 | ✓ (行 144+) |
| 6 | Order/Shape/Glaze/Result 面板不受影响 | ✓ |

**Risks**:
1. ArtRoot_Firing 节点被误删时 Builder 会回退到旧逻辑——需确保场景中 ArtRoot_Firing 不被删除
2. 检测依赖节点名 `ArtRoot_Firing` 严格匹配，大小写敏感
3. 若同时存在 ArtRoot_Firing 和旧占位控件，Builder 不会清理旧控件——当前场景旧控件已 inactive，无冲突

---

### Phase3 烧制 UI 接入工作流 — COMPLETE 2026-06-19

**整体成果**: 6 个 Task 全部 PASS，烧制 UI 美术资源接入完成。

| Task | 状态 | 核心成果 |
|------|------|---------|
| Task01 FiringSystem 扩展 | PASS | 低风门降温 + IsTemperatureDropping/WindValue/ForceUnderfiredOpen |
| Task02 场景层级搭建 | PASS | ArtRoot_Firing + 4 VideoPlayer + 2 RenderTexture + 风门 + 按钮换皮 |
| Task03 FiringPanelController 状态机 | PASS | 3态状态机(force参数) + 视频两段式 + Refresh职责分离 + 13字段绑定 |
| Task04 风门滚轮交互 | PASS | 滚轮9档切换 + 序列帧缓存 + Overlay模式检测 |
| Task05 温度条视频驱动 | PASS | temperatureBarProgress独立 + 倒放 + 强制开窑次品 |
| Task06 Phase3SceneBuilder 检测分支 | PASS | ArtRoot_Firing 检测，美术模式跳过旧占位控件 |
...
**待后续优化项**:
- 起势视频潜在重播（Task04 风险，观察后再收紧）
- 余热 decay window（Task05 风险A，观察后再加）
- 温度条视频 seek 性能（若卡顿降低 seek 频率）
- 触屏设备滚轮适配（第一版不处理）
